import numpy as np
import pandas as pd
from resistance_calculations import get_input_segment, totalResistanceForces, tractive_power
import string
from openpyxl.utils import get_column_letter


def compute_energy_matrix_with_stops(stations_m, assumptions):
    n = len(stations_m)
    ...
    for i in range(n):
        for j in range(n):
            ...
            print(f"Simulating from station {i} ({start} m) to station {j} ({end} m)...")
            print(start)
            print(end)
            # Get simulation results
            acc, vel, grade, curv, dist = get_input_segment(start, end, assumptions)
            print("babbaba")

            resistance = totalResistanceForces(vel, grade, curv, assumptions)
            _, _, _, _, _, total_energy_kwh, regen_buffer = tractive_power(acc, resistance, vel, assumptions)
            print('hellow')

            ### /7000 for battery
            energy_matrix[i][j] = total_energy_kwh/7000
    print("gret!")

    return energy_matrix

def compute_time_matrix(stations_m, assumptions):
    n = len(stations_m)
    ...
    for i in range(n):
        for j in range(n):
            if i == j:
                ...
                _, _, _, _, distance_seconds = get_input_segment(stations_m[i], stations_m[j], assumptions)
...
def export_energy_matrix_to_excel(matrix, stations_m, output_path):
