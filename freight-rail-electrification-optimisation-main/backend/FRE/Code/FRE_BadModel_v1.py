...
for id_instance_random in id_instance_random_list:
    ...
    combine_file_path = os.path.join(base_dir,'Results', 'Random Instances', 'CombineResults_Random.xlsx')
    ...
    = BadModel_change_rate_v2_PLARec(Stations, Trains, Containers, Power_train, TravelTime_train, hour_battery_swap, rate_charge_empty, penalty_delay, penalty_fix_cost, \
    ...
    '''export results to combined result book'''
    combinebook = openpyxl.load_workbook(combine_file_path)
    resultsheet = combinebook.get_sheet_by_name("CombineResults_%s" %alg + "_Delay%d" %(100*penalty_delay))
    r = resultsheet.max_row + 1 
    resultsheet.cell(row = r, column = 1).value = id_instance_random
    resultsheet.cell(row = r, column = 2).value = len(Stations)
    resultsheet.cell(row = r, column = 3).value = len(Trains)
    resultsheet.cell(row = r, column = 4).value = sum(len(Trains[i]['containers']) for i in Trains)/float(len(Trains))
    resultsheet.cell(row = r, column = 5).value = penalty_fix_cost
    resultsheet.cell(row = r, column = 6).value = penalty_delay
    resultsheet.cell(row = r, column = 7).value = obj_ub
    resultsheet.cell(row = r, column = 8).value = sum(X[i] for i in Stations)
    resultsheet.cell(row = r, column = 9).value = sum((Stations[i]['cost_fix']*X[i]) for i in Stations)
    resultsheet.cell(row = r, column = 10).value = sum(sum((D[i][j] - Trains[j]['stations'][i]['time_wait']) \
                                                          for j in Trains) \
                                                      for i in Stations) \
                                                    / len(Trains)
    resultsheet.cell(row = r, column = 11).value = sum(sum(Y[j][k] for k in Trains[j]['containers']) for j in Trains) / float(len(Trains))
    resultsheet.cell(row = r, column = 12).value = sum(sum(sum(T_c[i][j][k] for k in Trains[j]['containers']) for j in Trains) for i in Stations) / float(len(Trains))
    resultsheet.cell(row = r, column = 13).value = hour_battery_swap * sum(sum(sum(Z_s[i][j][k] for k in Trains[j]['containers']) for j in Trains) for i in Stations) / float(len(Trains))
    resultsheet.cell(row = r, column = 14).value = sum(sum(sum(T_c[i][j][k] for k in Trains[j]['containers']) for j in Trains) for i in Stations) / sum(X[i] for i in Stations)
    resultsheet.cell(row = r, column = 15).value = hour_battery_swap * sum(sum(sum(Z_s[i][j][k] for k in Trains[j]['containers']) for j in Trains) for i in Stations) / sum(X[i] for i in Stations)
    resultsheet.cell(row = r, column = 16).value = time_model
    resultsheet.cell(row = r, column = 17).value = lower_bound
    resultsheet.cell(row = r, column = 18).value = 100 * (obj_ub-lower_bound)/obj_ub
    
    combinebook.save(combine_file_path)
    combinebook.close()
