...
'''========================================================================================================================================================'''
''' This model is same as Model_change_rate_v2_PLARec except:
    (1) This model maximizes objective function
    (2) It adds a budget limit
    (3) It adds an upper limit on the total delay time
'''
def BadModel_change_rate_v2_PLARec(Stations, Trains, Containers, Power_train, TravelTime_train, hour_battery_swap, rate_charge_empty, penalty_delay, penalty_fix_cost, \
                                   budget, limit_delay, M, epsilon, buffer_soc_estimate, Segments_SOC, Segments_hour_charge, gap_op, time_limit_op, mysolver):

    time_start = time.time()
    badmodel_change_rate_v2_PLARec = pe.ConcreteModel()
    # print ('Defining indices and sets...')
    # Sets and indices
    badmodel_change_rate_v2_PLARec.i = pe.Set(initialize = sorted(set(Stations.keys())))
    badmodel_change_rate_v2_PLARec.i1 = pe.Set(initialize = sorted(set(Stations.keys())))
    badmodel_change_rate_v2_PLARec.i2 = pe.Set(initialize = sorted(set(Stations.keys())))
    badmodel_change_rate_v2_PLARec.j = pe.Set(initialize = sorted(set(Trains.keys())))
    badmodel_change_rate_v2_PLARec.k = pe.Set(initialize = sorted(set(Containers.keys())))
    badmodel_change_rate_v2_PLARec.k1 = pe.Set(initialize = sorted(set(Containers.keys())))
    badmodel_change_rate_v2_PLARec.k2 = pe.Set(initialize = sorted(set(Containers.keys())))
    badmodel_change_rate_v2_PLARec.u = pe.Set(initialize = sorted(set(Segments_SOC.keys())))
    badmodel_change_rate_v2_PLARec.v = pe.Set(initialize = sorted(set(Segments_hour_charge.keys())))
    
    # Variables
    # print ('Defining variables...')
    badmodel_change_rate_v2_PLARec.D = pe.Var(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, within = pe.NonNegativeReals)
    badmodel_change_rate_v2_PLARec.S_arrive = pe.Var(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, within = pe.NonNegativeReals)
    badmodel_change_rate_v2_PLARec.S_depart = pe.Var(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, within = pe.NonNegativeReals)
    badmodel_change_rate_v2_PLARec.T_arrive = pe.Var(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, within = pe.NonNegativeReals)
    badmodel_change_rate_v2_PLARec.T_depart = pe.Var(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, within = pe.NonNegativeReals)
    badmodel_change_rate_v2_PLARec.X = pe.Var(badmodel_change_rate_v2_PLARec.i, within = pe.Binary)
    badmodel_change_rate_v2_PLARec.Y = pe.Var(badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, within = pe.Binary)
    badmodel_change_rate_v2_PLARec.Z_c = pe.Var(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, within = pe.Binary)
    badmodel_change_rate_v2_PLARec.Z_s = pe.Var(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, within = pe.Binary)
    badmodel_change_rate_v2_PLARec.T_c = pe.Var(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, within = pe.NonNegativeReals)  # the amount of charging time of the battery in container k train j at station i
    # variables for linearization
    badmodel_change_rate_v2_PLARec.B = pe.Var(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, within = pe.Binary)  # binary variables to linearize constraints battery_sequential_rule
    badmodel_change_rate_v2_PLARec.F = pe.Var(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, within = pe.NonNegativeReals)  # in PLA, nonnegative continuous variables to replace (1 - S_arrive) * (1-r)**T_c
    badmodel_change_rate_v2_PLARec.beta = pe.Var(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, badmodel_change_rate_v2_PLARec.u, within = pe.Binary)  # in PLA, binary variables for S_arrive
    badmodel_change_rate_v2_PLARec.gamma = pe.Var(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, badmodel_change_rate_v2_PLARec.u, within = pe.NonNegativeReals)  # in PLA, continuous variables for S_arrive
    badmodel_change_rate_v2_PLARec.tau = pe.Var(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, badmodel_change_rate_v2_PLARec.v, within = pe.Binary)  # in PLA, binary variables for T_c
    badmodel_change_rate_v2_PLARec.eta = pe.Var(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, badmodel_change_rate_v2_PLARec.v, within = pe.NonNegativeReals)  # in PLA, continuous variables for T_c


    # preprocessing  # since it is AbstractModel, we move preprocessing constraints to formal constraints
    # print ('Preprocessing') 
    # If container k does not belong to train j, then Y[j,k] = 0. This rule replaces constraints "Upper bound on the number of batteries that train j can carry" in the latex badmodel_change_rate_v2_PLARec.
    for j in badmodel_change_rate_v2_PLARec.j:
        for k in badmodel_change_rate_v2_PLARec.k:
            if k not in Trains[j]['containers']:
                badmodel_change_rate_v2_PLARec.Y[j,k].fix(0)
            else:
                # to minimize the number of stations, we assign as many batteries to each train as possible, so each consist carries a battery
                badmodel_change_rate_v2_PLARec.Y[j,k].fix(1)
    # For each train j, the depart and arrival time at the origin are both zero.
    for j in badmodel_change_rate_v2_PLARec.j:
        badmodel_change_rate_v2_PLARec.T_arrive['origin',j].fix(0)
        badmodel_change_rate_v2_PLARec.T_depart['origin',j].fix(0)
    
    # objective function
    # print ('Reading objective...')
    def obj_rule(badmodel_change_rate_v2_PLARec):
        cost_fix = sum(Stations[i]['cost_fix'] * badmodel_change_rate_v2_PLARec.X[i] for i in badmodel_change_rate_v2_PLARec.i)
        cost_delay = sum(sum((badmodel_change_rate_v2_PLARec.D[i,j] - Trains[j]['stations'][i]['time_wait']) for j in badmodel_change_rate_v2_PLARec.j) for i in badmodel_change_rate_v2_PLARec.i)
        obj = penalty_fix_cost * cost_fix + penalty_delay * cost_delay
        return obj  
    badmodel_change_rate_v2_PLARec.obj = pe.Objective(rule = obj_rule, sense = pe.maximize)
    
    
    # constraints
    # budget constraint
    def budget_rule(badmodel_change_rate_v2_PLARec):
        return sum(Stations[i]['cost_fix'] * badmodel_change_rate_v2_PLARec.X[i] for i in badmodel_change_rate_v2_PLARec.i) <= budget
    badmodel_change_rate_v2_PLARec.budget_rule = pe.Constraint(rule = budget_rule)
 
    # upper limit on total delay time
    def limit_delay_rule(badmodel_change_rate_v2_PLARec):
        return sum(sum((badmodel_change_rate_v2_PLARec.D[i,j] - Trains[j]['stations'][i]['time_wait']) \
                       for j in badmodel_change_rate_v2_PLARec.j) \
                   for i in badmodel_change_rate_v2_PLARec.i) \
               <= limit_delay
    badmodel_change_rate_v2_PLARec.limit_delay_rule = pe.Constraint(rule = limit_delay_rule)
 
    # define delay time for train j in station i
    # print ('a...')
    def delay_define_rule(badmodel_change_rate_v2_PLARec, i, j):
        return badmodel_change_rate_v2_PLARec.D[i,j] >= badmodel_change_rate_v2_PLARec.T_depart[i,j] - badmodel_change_rate_v2_PLARec.T_arrive[i,j]
    badmodel_change_rate_v2_PLARec.delay_define_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, rule = delay_define_rule)
    
    # If station is not deployed, then batteries can be neither swapped nor charged in station i
    # print ('b...')
    def deploy_swap_charge_rule(badmodel_change_rate_v2_PLARec, i):
        return sum(sum((badmodel_change_rate_v2_PLARec.Z_c[i,j,k] + badmodel_change_rate_v2_PLARec.Z_s[i,j,k]) \
                       for k in badmodel_change_rate_v2_PLARec.k if k in Trains[j]['containers']) \
                   for j in badmodel_change_rate_v2_PLARec.j) \
                <= 2 * sum(sum(1 for k in Containers if k in Trains[j]['containers']) for j in Trains) * badmodel_change_rate_v2_PLARec.X[i]
    badmodel_change_rate_v2_PLARec.deploy_swap_charge_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.i, rule = deploy_swap_charge_rule)        
    
    # # If station is deployed, then at least one battery must be swapped nor charged in station i
    # print ('opt2...')
    def deploy_swap_charge_must_rule_orig_feas(badmodel_change_rate_v2_PLARec, i):
        return sum(sum((badmodel_change_rate_v2_PLARec.Z_c[i,j,k] + badmodel_change_rate_v2_PLARec.Z_s[i,j,k]) \
                        for k in badmodel_change_rate_v2_PLARec.k if k in Trains[j]['containers']) \
                    for j in badmodel_change_rate_v2_PLARec.j) \
                >= badmodel_change_rate_v2_PLARec.X[i]
    badmodel_change_rate_v2_PLARec.deploy_swap_charge_must_rule_orig_feas = pe.Constraint(badmodel_change_rate_v2_PLARec.i, rule = deploy_swap_charge_must_rule_orig_feas)   

    # Train j cannot both swap and charge batteries in station i.
    # print ('c...')
    def noboth_swap_charge_rule(badmodel_change_rate_v2_PLARec, i, j, k1, k2):
        if k1 in Trains[j]['containers'] and k2 in Trains[j]['containers']:
            return badmodel_change_rate_v2_PLARec.Z_s[i,j,k1] + badmodel_change_rate_v2_PLARec.Z_c[i,j,k2] <= 1
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.noboth_swap_charge_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k1, badmodel_change_rate_v2_PLARec.k2, rule = noboth_swap_charge_rule)        
    
    # Train j cannot depart station i until passenger trains have passed.
    # print ('d...')
    def wait_passenger_rule(badmodel_change_rate_v2_PLARec, i, j):
        return badmodel_change_rate_v2_PLARec.T_depart[i,j] - badmodel_change_rate_v2_PLARec.T_arrive[i,j] >= Trains[j]['stations'][i]['time_wait']
    badmodel_change_rate_v2_PLARec.wait_passenger_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, rule = wait_passenger_rule)        
    ...
    # In train j, if container k does not carry a battery, then all the containers behind k do not carry batteries, either. In other words, for each train, batteries can only be stored in the first few consecutive containers. Assume the containers closer to the locomotive have smaller indices.
    # print ('e0...')
    def consecutive_battery_rule(badmodel_change_rate_v2_PLARec, j, k):
        if k in Trains[j]['containers']:
            kid = int(((k.split('container '))[1].split(' in')[0]))
            if kid < len(Trains[j]['containers']):
                return sum(badmodel_change_rate_v2_PLARec.Y[j,kp] \
                           for kp in badmodel_change_rate_v2_PLARec.k if kp in Trains[j]['containers'] and int(((kp.split('container '))[1].split(' in')[0])) >= kid+1) \
                       <= sum(1 for kp in badmodel_change_rate_v2_PLARec.k if kp in Trains[j]['containers'] and int(((kp.split('container '))[1].split(' in')[0])) >= kid+1) \
                          * badmodel_change_rate_v2_PLARec.Y[j,k]
            else:
                return pe.Constraint.Skip
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.consecutive_battery_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = consecutive_battery_rule)  
    
    # If train j chooses to swap batteries in station i, the number of swapped batteries cannot exceed the number of available batterries in i.
    # print ('e...')
    def max_number_batteries_station_rule(badmodel_change_rate_v2_PLARec, i, j):
        return sum(badmodel_change_rate_v2_PLARec.Z_s[i,j,k] for k in Trains[j]['containers']) <= Stations[i]['max_number_batteries']
    badmodel_change_rate_v2_PLARec.max_number_batteries_station_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, rule = max_number_batteries_station_rule)  
    
    # if train j chooses to charge batteries in station i, the number of charged batteries cannot exceed teh number of available chargers in i.
    # print ('f...')
    def max_number_chargers_station_rule(badmodel_change_rate_v2_PLARec, i, j):
        return sum(badmodel_change_rate_v2_PLARec.Z_c[i,j,k] for k in Trains[j]['containers']) <= Stations[i]['max_number_chargers']
    badmodel_change_rate_v2_PLARec.max_number_chargers_station_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, rule = max_number_chargers_station_rule)  
    
    # If station i2 is immediately after station i1 along the route, then for train j, the SOC at its arrival at i2 must equal the SOC at its departure from i1 minus the amount of power required for train j to travel from stations i1 to i2.
    # print ('g...')
    def power_rule(badmodel_change_rate_v2_PLARec, i1, i2, j):
        if i2 == Stations[i1]['station_after']:
            return sum(badmodel_change_rate_v2_PLARec.S_depart[i1,j,k] for k in Trains[j]['containers']) - sum(badmodel_change_rate_v2_PLARec.S_arrive[i2,j,k] for k in Trains[j]['containers']) == Power_train[j][i1][i2]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.power_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.i1, badmodel_change_rate_v2_PLARec.i2, badmodel_change_rate_v2_PLARec.j, rule = power_rule)          
    
    # For each train j, electricity in each battery is consumed sequentially, starting from the battery closest to the locomotive. If container k1 is closer to the locomotive than k2, then the battery in k2 will not be used before the battery in k1 is run out.
    # print ('g1...')
    def battery_sequential_rule_linear_a(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            k_index = Trains[j]['containers'].index(k)
            if k_index < len(Trains[j]['containers'])-1:
                return badmodel_change_rate_v2_PLARec.S_arrive[i,j,k]  -  M * badmodel_change_rate_v2_PLARec.B[i,j,k]  <=  0
            else:
                return pe.Constraint.Skip
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.battery_sequential_rule_linear_a = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = battery_sequential_rule_linear_a)          

    # print ('g2...')
    def battery_sequential_rule_linear_b(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            k_index = Trains[j]['containers'].index(k)
            if k_index < len(Trains[j]['containers'])-1:
                k_next = Trains[j]['containers'][k_index+1]
                return (1 - badmodel_change_rate_v2_PLARec.S_arrive[i,j,k_next])  +  M * badmodel_change_rate_v2_PLARec.B[i,j,k] <=  M
            else:
                return pe.Constraint.Skip
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.battery_sequential_rule_linear_b = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = battery_sequential_rule_linear_b)          
    
    # When train j departs station i, the SOC of battery in container k of train j cannot be lower than the SOC at its arrival.
    # print ('h...')
    def soc_increase_rule(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            return badmodel_change_rate_v2_PLARec.S_depart[i,j,k] - badmodel_change_rate_v2_PLARec.S_arrive[i,j,k] >= 0
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_increase_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = soc_increase_rule)          
    
    # If the battery in container k of train j is swapped in station i, then the SOC is 100% when train j leaves station i.
    # print ('i...')
    def swap_full_soc_rule(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            return badmodel_change_rate_v2_PLARec.S_depart[i,j,k] >= badmodel_change_rate_v2_PLARec.Z_s[i,j,k]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.swap_full_soc_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = swap_full_soc_rule)     
    
    # If station i2 is immediately after i1, then for each battery in container k of train j, the SOC at its arrival in station i2 must be no higher than that at its departure from station i1.
    # print ('i1...')
    def soc_depart_arrive_between_stations_rule(badmodel_change_rate_v2_PLARec, i1, i2, j, k):
        if k in Trains[j]['containers'] and i2 == Stations[i1]['station_after']:
            return badmodel_change_rate_v2_PLARec.S_arrive[i2,j,k] - badmodel_change_rate_v2_PLARec.S_depart[i1,j,k] <= 0
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_depart_arrive_between_stations_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.i1, badmodel_change_rate_v2_PLARec.i2, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = soc_depart_arrive_between_stations_rule)     

    # If the battery in container k of train j is swapped in station i, then train j must stay in i for at least      
    # print ('j...')
    def time_battery_swap_rule(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            return badmodel_change_rate_v2_PLARec.T_depart[i,j] - badmodel_change_rate_v2_PLARec.T_arrive[i,j] >= hour_battery_swap * badmodel_change_rate_v2_PLARec.Z_s[i,j,k]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.time_battery_swap_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = time_battery_swap_rule)     
    
    # If the battery in container k of train j is charged in station i, then when train j leaves station i, the SOC of container k is a function of the SOC when j arrives at i, and the charging time.
    # print ('k0...')
    def soc_time_charge_rule_PLA_Zs(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            return badmodel_change_rate_v2_PLARec.S_depart[i,j,k] \
                   + badmodel_change_rate_v2_PLARec.F[i,j,k] \
                   - M * badmodel_change_rate_v2_PLARec.Z_s[i,j,k] \
                   <= 1
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_time_charge_rule_PLA_Zs = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = soc_time_charge_rule_PLA_Zs)        
    
    # print ('k1...')
    def soc_time_charge_rule_PLA_Zc(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            return badmodel_change_rate_v2_PLARec.S_depart[i,j,k] \
                   + badmodel_change_rate_v2_PLARec.F[i,j,k] \
                   + epsilon * badmodel_change_rate_v2_PLARec.Z_c[i,j,k] \
                   >= 1
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_time_charge_rule_PLA_Zc = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = soc_time_charge_rule_PLA_Zc)        

    # print ('k2...')
    def soc_time_charge_rule_PLA_beta1(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            return sum(badmodel_change_rate_v2_PLARec.beta[i,j,k,u] for u in badmodel_change_rate_v2_PLARec.u if u != max(badmodel_change_rate_v2_PLARec.u)) == 1
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_time_charge_rule_PLA_beta1 = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = soc_time_charge_rule_PLA_beta1)        

    # print ('k3...')
    def soc_time_charge_rule_PLA_gamma1(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            return sum(badmodel_change_rate_v2_PLARec.gamma[i,j,k,u] for u in badmodel_change_rate_v2_PLARec.u) == 1
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_time_charge_rule_PLA_gamma1 = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = soc_time_charge_rule_PLA_gamma1)        

    # print ('k4...')
    def soc_time_charge_rule_PLA_tau1(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            return sum(badmodel_change_rate_v2_PLARec.tau[i,j,k,v] for v in badmodel_change_rate_v2_PLARec.v if v != max(badmodel_change_rate_v2_PLARec.v)) == 1
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_time_charge_rule_PLA_tau1 = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = soc_time_charge_rule_PLA_tau1)        

    # print ('k5...')
    def soc_time_charge_rule_PLA_gamma_Sarrive(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            return sum((Segments_SOC[u]['SOC'] * badmodel_change_rate_v2_PLARec.gamma[i,j,k,u]) \
                       for u in badmodel_change_rate_v2_PLARec.u) \
                   == badmodel_change_rate_v2_PLARec.S_arrive[i,j,k]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_time_charge_rule_PLA_gamma_Sarrive = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = soc_time_charge_rule_PLA_gamma_Sarrive)        

    # print ('k6...')
    def soc_time_charge_rule_PLA_tau_eta_Tc(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            return sum((Segments_hour_charge[v]['hour'] * badmodel_change_rate_v2_PLARec.tau[i,j,k,v] \
                        + (Segments_hour_charge[v+1]['hour']-Segments_hour_charge[v]['hour']) * badmodel_change_rate_v2_PLARec.eta[i,j,k,v]) \
                       for v in badmodel_change_rate_v2_PLARec.v if v != max(badmodel_change_rate_v2_PLARec.v)) \
                   == badmodel_change_rate_v2_PLARec.T_c[i,j,k]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_time_charge_rule_PLA_tau_eta_Tc = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = soc_time_charge_rule_PLA_tau_eta_Tc)        

    # print ('k7...')
    def soc_time_charge_rule_PLA_gamma_beta(badmodel_change_rate_v2_PLARec, i, j, k, u):
        if k in Trains[j]['containers'] and u != min(badmodel_change_rate_v2_PLARec.u) and u != max(badmodel_change_rate_v2_PLARec.u):
            return badmodel_change_rate_v2_PLARec.gamma[i,j,k,u] <= badmodel_change_rate_v2_PLARec.beta[i,j,k,u-1] + badmodel_change_rate_v2_PLARec.beta[i,j,k,u]
        elif k in Trains[j]['containers'] and u == min(badmodel_change_rate_v2_PLARec.u):
            return badmodel_change_rate_v2_PLARec.gamma[i,j,k,u] <= badmodel_change_rate_v2_PLARec.beta[i,j,k,u]
        elif k in Trains[j]['containers'] and u == max(badmodel_change_rate_v2_PLARec.u):
            return badmodel_change_rate_v2_PLARec.gamma[i,j,k,u] <= badmodel_change_rate_v2_PLARec.beta[i,j,k,u-1]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_time_charge_rule_PLA_gamma_beta = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, badmodel_change_rate_v2_PLARec.u, rule = soc_time_charge_rule_PLA_gamma_beta)        

    # print ('k8...')
    def soc_time_charge_rule_PLA_eta_tau(badmodel_change_rate_v2_PLARec, i, j, k, v):
        if k in Trains[j]['containers'] and v != min(badmodel_change_rate_v2_PLARec.v) and v != max(badmodel_change_rate_v2_PLARec.v):
            return badmodel_change_rate_v2_PLARec.eta[i,j,k,v] <= badmodel_change_rate_v2_PLARec.tau[i,j,k,v-1] + badmodel_change_rate_v2_PLARec.tau[i,j,k,v]
        elif k in Trains[j]['containers'] and v == min(badmodel_change_rate_v2_PLARec.v):
            return badmodel_change_rate_v2_PLARec.eta[i,j,k,v] <= badmodel_change_rate_v2_PLARec.tau[i,j,k,v]
        elif k in Trains[j]['containers'] and v == max(badmodel_change_rate_v2_PLARec.v):
            return badmodel_change_rate_v2_PLARec.eta[i,j,k,v] <= badmodel_change_rate_v2_PLARec.tau[i,j,k,v-1]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_time_charge_rule_PLA_eta_tau = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, badmodel_change_rate_v2_PLARec.v, rule = soc_time_charge_rule_PLA_eta_tau)        

    # print ('k4...')
    def soc_time_charge_rule_PLA_eta_tau1(badmodel_change_rate_v2_PLARec, i, j, k, v):
        if k in Trains[j]['containers'] and v != min(badmodel_change_rate_v2_PLARec.v) and v != max(badmodel_change_rate_v2_PLARec.v):
            return badmodel_change_rate_v2_PLARec.eta[i,j,k,v] <= badmodel_change_rate_v2_PLARec.tau[i,j,k,v]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_time_charge_rule_PLA_eta_tau1 = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, badmodel_change_rate_v2_PLARec.v, rule = soc_time_charge_rule_PLA_eta_tau1)        

    # print ('k9...')
    def soc_time_charge_rule_PLA_F_leq(badmodel_change_rate_v2_PLARec, i, j, k, u, v):
        if k in Trains[j]['containers'] and u != max(badmodel_change_rate_v2_PLARec.u) and v != max(badmodel_change_rate_v2_PLARec.v):
            s0, t0 = Segments_SOC[u]['SOC'], Segments_hour_charge[v]['hour']
            s1, t1 = Segments_SOC[u+1]['SOC'], Segments_hour_charge[v+1]['hour']
            g_0_0 = (1-s0) * ((1-rate_charge_empty)**t0)
            g_0_1 = (1-s0) * ((1-rate_charge_empty)**t1)
            ...
            g_1_1 = (1-s1) * ((1-rate_charge_empty)**t1)
            w = min(g_0_1 - g_0_0, g_1_1 - g_1_0)
            return badmodel_change_rate_v2_PLARec.F[i,j,k] \
                   <= sum((badmodel_change_rate_v2_PLARec.gamma[i,j,k,w] * ((1-Segments_SOC[w]['SOC'])*((1-rate_charge_empty)**t0) )) \
                          for w in badmodel_change_rate_v2_PLARec.u) \
                      + w * badmodel_change_rate_v2_PLARec.eta[i,j,k,v] \
                      + M * (2 - badmodel_change_rate_v2_PLARec.tau[i,j,k,v] - badmodel_change_rate_v2_PLARec.beta[i,j,k,u])
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_time_charge_rule_PLA_F_leq = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, badmodel_change_rate_v2_PLARec.u, badmodel_change_rate_v2_PLARec.v, rule = soc_time_charge_rule_PLA_F_leq)        

    # print ('k10...')
    def soc_time_charge_rule_PLA_F_geq(badmodel_change_rate_v2_PLARec, i, j, k, u, v):
        if k in Trains[j]['containers'] and u != max(badmodel_change_rate_v2_PLARec.u) and v != max(badmodel_change_rate_v2_PLARec.v):
            s0, t0 = Segments_SOC[u]['SOC'], Segments_hour_charge[v]['hour']
            s1, t1 = Segments_SOC[u+1]['SOC'], Segments_hour_charge[v+1]['hour']
            g_0_0 = (1-s0) * ((1-rate_charge_empty)**t0)
            ...
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_time_charge_rule_PLA_F_geq = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, badmodel_change_rate_v2_PLARec.u, badmodel_change_rate_v2_PLARec.v, rule = soc_time_charge_rule_PLA_F_geq)        

    # print ('l...')
    # def soc_time_charge_rule_b(badmodel_change_rate_v2_PLARec, i, j, k):
    #     if k in Trains[j]['containers']:
    #         return rate_charge * badmodel_change_rate_v2_PLARec.T_c[i,j,k] <= 1 - badmodel_change_rate_v2_PLARec.S_arrive[i,j,k]
    #     else:
    #         return pe.Constraint.Skip
    # badmodel_change_rate_v2_PLARec.soc_time_charge_rule_b = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = soc_time_charge_rule_b)
    
    # print ('m...')
    def soc_time_charge_rule_c(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            return badmodel_change_rate_v2_PLARec.T_c[i,j,k] <= badmodel_change_rate_v2_PLARec.T_depart[i,j] - badmodel_change_rate_v2_PLARec.T_arrive[i,j]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_time_charge_rule_c = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = soc_time_charge_rule_c)
    
    # print ('n...')
    def soc_time_charge_rule_d(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            return badmodel_change_rate_v2_PLARec.T_c[i,j,k] <= M * badmodel_change_rate_v2_PLARec.Z_c[i,j,k]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_time_charge_rule_d = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = soc_time_charge_rule_d)
    
    # print ('n1...')
    def soc_time_charge_rule_e(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            return badmodel_change_rate_v2_PLARec.Z_c[i,j,k] <= M * badmodel_change_rate_v2_PLARec.T_c[i,j,k]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_time_charge_rule_e = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = soc_time_charge_rule_e)
    
    # If the battery in container k of train j is neither charged nor swapped in station i, then the SOC at its departure an arrival should be same.
    # print ('o...')
    def no_swap_charge_soc_rule(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            return badmodel_change_rate_v2_PLARec.S_depart[i,j,k] - badmodel_change_rate_v2_PLARec.S_arrive[i,j,k] <= badmodel_change_rate_v2_PLARec.Z_c[i,j,k] + badmodel_change_rate_v2_PLARec.Z_s[i,j,k]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.no_swap_charge_soc_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = no_swap_charge_soc_rule)
    
    # If container k of train j carries a battery, then it is assumed to be fully charged when the train departs the origin.
    # print ('o2...')
    def soc_depart_origin_rule(badmodel_change_rate_v2_PLARec, j, k):
        if k in Trains[j]['containers']:
            return badmodel_change_rate_v2_PLARec.S_depart['origin',j,k] >= badmodel_change_rate_v2_PLARec.Y[j,k]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_depart_origin_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = soc_depart_origin_rule)
    
    # To reduce CPLEX processing time, if container k of train j carries a battery, then its SOC is assumed to be 100% when the train arrives the origin.
    # print ('o3...')
    def soc_arrive_origin_rule(badmodel_change_rate_v2_PLARec, j, k):
        if k in Trains[j]['containers']:
            return badmodel_change_rate_v2_PLARec.S_arrive['origin',j,k] >= badmodel_change_rate_v2_PLARec.Y[j,k]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.soc_arrive_origin_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = soc_arrive_origin_rule)
    
    # If container k of train j carries a battery, then the battery is neither charged nor swapped at the origin.
    # print ('o4...')
    def origin_no_chargeswap_rule(badmodel_change_rate_v2_PLARec, j, k):
        if k in Trains[j]['containers']:
            return badmodel_change_rate_v2_PLARec.Z_c['origin',j,k] + badmodel_change_rate_v2_PLARec.Z_s['origin',j,k] <= 2 * badmodel_change_rate_v2_PLARec.Y[j,k]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.origin_no_chargeswap_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = origin_no_chargeswap_rule)
    
    # For train j, if station i2 is immediately after station i1 on the route, then the arrival time at i2 should equal the departure time at i1 plus the travel time from stations i1 to i2.
    # print ('o5...')
    def T_traveltime_rule(badmodel_change_rate_v2_PLARec, i1, i2, j):
        if i2 == Stations[i1]['station_after']:
            return badmodel_change_rate_v2_PLARec.T_arrive[i2,j] == badmodel_change_rate_v2_PLARec.T_depart[i1,j] + TravelTime_train[j][i1][i2]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.T_traveltime_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.i1, badmodel_change_rate_v2_PLARec.i2, badmodel_change_rate_v2_PLARec.j, rule = T_traveltime_rule)
    
    # If container k in train j does not carry a battery, then we can neither charge nor swap the battery of container k in train j at any stations.
    # print ('o6...')
    def nobattery_nochargeswap_rule(badmodel_change_rate_v2_PLARec, j, k):
        if k in Trains[j]['containers']:
            return sum(badmodel_change_rate_v2_PLARec.Z_c[i,j,k] for i in badmodel_change_rate_v2_PLARec.i) + sum(badmodel_change_rate_v2_PLARec.Z_s[i,j,k] for i in badmodel_change_rate_v2_PLARec.i) <= 2*len(Stations) * badmodel_change_rate_v2_PLARec.Y[j,k]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.nobattery_nochargeswap_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = nobattery_nochargeswap_rule)
    
    # If container k of train j does not carry a battery, then the SOC of the "dummy" battery in container k of train j must equal zero at all stations.
    # print ('o7...')
    def nobattery_zerosoc_rule(badmodel_change_rate_v2_PLARec, j, k):
        if k in Trains[j]['containers']:
            return sum(badmodel_change_rate_v2_PLARec.S_arrive[i,j,k] for i in badmodel_change_rate_v2_PLARec.i) + sum(badmodel_change_rate_v2_PLARec.S_depart[i,j,k] for i in badmodel_change_rate_v2_PLARec.i) <= 2*len(Stations) * badmodel_change_rate_v2_PLARec.Y[j,k]
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.nobattery_zerosoc_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = nobattery_zerosoc_rule)
    
    # The SOC of each battery must not exceed 100%.
    # print ('p...')
    def ub_soc_arrive_rule(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            return badmodel_change_rate_v2_PLARec.S_arrive[i,j,k] <= 1
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.ub_soc_arrive_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = ub_soc_arrive_rule)
    
    # print ('q...')
    def ub_soc_depart_rule(badmodel_change_rate_v2_PLARec, i, j, k):
        if k in Trains[j]['containers']:
            return badmodel_change_rate_v2_PLARec.S_depart[i,j,k] <= 1
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.ub_soc_depart_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = ub_soc_depart_rule)
    
    # print ('r...')
    # If container k does not belong to train j, then Y[j,k] = 0. This rule replaces constraints "Upper bound on the number of batteries that train j can carry" in the latex badmodel_change_rate_v2_PLARec.
    def preprocessing_Y0_rule(badmodel_change_rate_v2_PLARec, j, k):
        if k not in Trains[j]['containers']:
            return badmodel_change_rate_v2_PLARec.Y[j,k] == 0
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.preprocessing_Y0_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, rule = preprocessing_Y0_rule)
       
    # print ('s...')
    # For each train j, the depart and arrival time at the origin are both zero.
    def preprocessing_T0_rule(badmodel_change_rate_v2_PLARec, j):
        return badmodel_change_rate_v2_PLARec.T_arrive['origin',j] + badmodel_change_rate_v2_PLARec.T_depart['origin',j] == 0
    badmodel_change_rate_v2_PLARec.preprocessing_T0_rule = pe.Constraint(badmodel_change_rate_v2_PLARec.j, rule = preprocessing_T0_rule)
      
    # the value of gamma an eta variables are no greater than 1
    # *** print ('t...')
    def ub_gamma_rule_orig(badmodel_change_rate_v2_PLARec, i, j, k, u):
        if k in Trains[j]['containers']:
            return - badmodel_change_rate_v2_PLARec.gamma[i,j,k,u] >= -1
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.ub_gamma_rule_orig = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, badmodel_change_rate_v2_PLARec.u, rule = ub_gamma_rule_orig)
    
    def ub_eta_rule_orig(badmodel_change_rate_v2_PLARec, i, j, k, v):
        if k in Trains[j]['containers']:
            return - badmodel_change_rate_v2_PLARec.eta[i,j,k,v] >= -1
        else:
            return pe.Constraint.Skip
    badmodel_change_rate_v2_PLARec.ub_eta_rule_orig = pe.Constraint(badmodel_change_rate_v2_PLARec.i, badmodel_change_rate_v2_PLARec.j, badmodel_change_rate_v2_PLARec.k, badmodel_change_rate_v2_PLARec.v, rule = ub_eta_rule_orig)
    ...
    if mysolver == 'cplex':
        ...
        opt.options["mip_tolerances_mipgap"] = gap_op
        opt.options["timelimit"] = 900
        results=opt.solve(badmodel_change_rate_v2_PLARec, tee=True, keepfiles=True)
        # results.write()
    
    # # solve by ipopt: treats the variables as continuous
    # solver = SolverFactory('ipopt')
    # #solver.options['max_iter']= 10000
    # results= solver.solve(badmodel_change_rate_v2_PLARec, tee=True)    
    # results.write()

    # opt=SolverFactory('apopt.py')
    # results=opt.solve(badmodel_change_rate_v2_PLARec)
    # results.write()
    # instance.load(results)
    ...
    # # opt.options['timeLimit'] = 120  # Set maximum time limit (in seconds)
    # # opt.options['logFile'] = 'couenne_log.txt'
    # results=opt.solve(badmodel_change_rate_v2_PLARec, timelimit=300, logfile = 'mylog.txt', tee=True)
    # results.write()
    ...
    # solve by gurobi:
        ...
        # solver.options['Cuts'] = 3  # Aggressive cutting planes
        # solver.options['VarBranch'] = 2  # Strong branching
        results = solver.solve(badmodel_change_rate_v2_PLARec, tee=True, keepfiles=True)
        results.write()
    ...
    # calculate obj
    cost_fix = sum(Stations[i]['cost_fix'] * badmodel_change_rate_v2_PLARec.X[i].value for i in badmodel_change_rate_v2_PLARec.i)
    cost_delay = sum(sum((badmodel_change_rate_v2_PLARec.D[i,j].value - Trains[j]['stations'][i]['time_wait']) for j in badmodel_change_rate_v2_PLARec.j) for i in badmodel_change_rate_v2_PLARec.i)
    obj = penalty_fix_cost * cost_fix + penalty_delay * cost_delay
    cost_fix_weight = penalty_fix_cost * cost_fix
    cost_delay_weight = penalty_delay * cost_delay
    ...
    for i in Stations:
        X.update({i: badmodel_change_rate_v2_PLARec.X[i].value})
        D.update({i: {}})
        S_arrive.update({i: {}})
        S_depart.update({i: {}})
        T_arrive.update({i: {}})
        ...
        F.update({i: {}})
        for j in Trains:
            D[i].update({j: badmodel_change_rate_v2_PLARec.D[i,j].value})
            T_arrive[i].update({j: badmodel_change_rate_v2_PLARec.T_arrive[i,j].value})
            T_depart[i].update({j: badmodel_change_rate_v2_PLARec.T_depart[i,j].value})
            S_arrive[i].update({j: {}})
            S_depart[i].update({j: {}})
            Z_c[i].update({j: {}})
            Z_s[i].update({j: {}})
            ...
            F[i].update({j: {}})
            for k in Trains[j]['containers']:
                S_arrive[i][j].update({k: badmodel_change_rate_v2_PLARec.S_arrive[i,j,k].value})
                S_depart[i][j].update({k: badmodel_change_rate_v2_PLARec.S_depart[i,j,k].value})
                Z_c[i][j].update({k: badmodel_change_rate_v2_PLARec.Z_c[i,j,k].value})
                Z_s[i][j].update({k: badmodel_change_rate_v2_PLARec.Z_s[i,j,k].value})
                T_c[i][j].update({k: badmodel_change_rate_v2_PLARec.T_c[i,j,k].value})
                F[i][j].update({k: badmodel_change_rate_v2_PLARec.F[i,j,k].value})
    for j in Trains:
        Y.update({j: {}})
        for k in Trains[j]['containers']:
            Y[j].update({k: badmodel_change_rate_v2_PLARec.Y[j,k].value})
    ...
    for i in Stations:
        ...
        for j in Trains:
            ...
            for k in Trains[j]['containers']:
                ...
                tau[i][j].update({k: {}})
                eta[i][j].update({k: {}})
                B[i][j].update({k: badmodel_change_rate_v2_PLARec.B[i,j,k].value})
                for u in Segments_SOC:
                    if u != max(Segments_SOC):
                        beta[i][j][k].update({u: badmodel_change_rate_v2_PLARec.beta[i,j,k,u].value})
                    gamma[i][j][k].update({u: badmodel_change_rate_v2_PLARec.gamma[i,j,k,u].value})
                for v in Segments_hour_charge:
                    if v != max(Segments_hour_charge):
                        tau[i][j][k].update({v: badmodel_change_rate_v2_PLARec.tau[i,j,k,v].value})
                    eta[i][j][k].update({v: badmodel_change_rate_v2_PLARec.eta[i,j,k,v].value})
