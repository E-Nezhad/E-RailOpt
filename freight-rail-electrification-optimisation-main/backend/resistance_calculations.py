...
def tractive_power(acceleration_seconds, resistance_seconds, velocity_seconds, assumptions):
    ...
    debug_log = []  # ⬅️ store per-step data here
    log_entries = []
    print("hellow there")
    for i in range(len(acceleration_seconds)):
        a = acceleration_seconds[i]
        ...
        if a < -1e-5:
            try:
                nregen = 1 / math.exp(abs(regen_efficiency/a))
                regen = power * nregen
            except OverflowError:
                regen = 0
        else:
        ...
        log_entries.append({
            ...
            'Acceleration (m/s²)': a,
            'Resistance (N)': r,
            'Tractive Power (W)': power,
            'Energy (kWh)': power_kWh,
            'Net Energy (kWh)': net_energy,
            'Regen Buffer (kWh)': regen_buffer
        })
        energy_consumption.append(net_energy)
        total_power_output += power
        total_regen_power += regen
    ...
    df_log.to_excel('./uploads/segment_energy_log.xlsx', index=False)
    print("📤 Segment energy log saved to './uploads/segment_energy_log.xlsx'")


    print(f"✅ Logged velocity and energy profile to 'velocity_energy_log.xlsx'")
    print(f"⚡ Total net energy: {total_energy_consumption:.3f} kWh | Regen buffer left: {regen_buffer:.3f} kWh\n")
