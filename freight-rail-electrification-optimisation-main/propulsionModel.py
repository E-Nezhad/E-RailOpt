...
fa_cl = 10
K_cl = 0.0055
total_mass = m_loc * num_locos + m_car * num_cars
train_efficiency = 0.85
regen_efficiency = 0.5



def resistance_force_single(v, G, C):
...
def totalResistanceForces(velocity_seconds, grade_seconds, curvature_seconds):
    ...
    for i in range(len(velocity_seconds)):
        ...
        resistance_seconds[i] = resistance_force_single(v, G, C)
...
def get_input(P):
    ...
    max_decel = 0.5  # m/s², braking deceleration
    brake_buffer = 30  # extra distance buffer before braking
    max_tractive_force = 300000  # N, max tractive force from locomotive
    max_acceleration = 1.0  # m/s², max allowed acceleration
    ...
    while distance_seconds[-1] < total_distance:
        ...
        # Resistance
        F_resist = resistance_force_single(v, G, C)
        ...
        # accelerate
            ...
            a = F_net / total_mass
            a = max(min(a, max_acceleration), -max_decel)
...
def get_input_segment(P, start_m, end_m):
...
##def tractive_power(acceleration_seconds, resistance_seconds, velocity_seconds):
    ...
    for a, r, v in zip(acceleration_seconds, resistance_seconds, velocity_seconds):
        
        # model 1 tractive force
        power = (total_mass * a + r) * v
        power = power/train_efficiency
        power_output.append(power)
        
        # model 2 regenrative braking - if decelerating, this will charge battery
        if a < 0:
            regen = 1 / math.exp(abs(regen_efficiency / a))
        else:
            regen = 0
        regen_power.append(regen)
        net_energy = (power - regen)/(1000*3600)
    ...
    # kWh at time t, kWh at time t, total kWh, total kWh
    return power_output, regen_power, energy_consumption, total_power_output, total_regen_power, total_energy_consumption
    
def tractive_power(acceleration_seconds, resistance_seconds, velocity_seconds, regen_buffer):
    if not (len(acceleration_seconds) == len(resistance_seconds) == len(velocity_seconds)):
        raise ValueError("Input lists must have the same length")
...
def main():
    ...
    P = 3000*1000 # Watts
    acceleration_seconds, velocity_seconds, grade_seconds, curvature_seconds, distance_seconds = get_input(P)
    resistance_seconds = totalResistanceForces(velocity_seconds, grade_seconds, curvature_seconds)
    power_seconds, regend_seconds, energy_seconds, total_power_output, total_regen_power, total_energy_consumption = tractive_power(acceleration_seconds, resistance_seconds, velocity_seconds)
    
    total = 0
    for i, (R,s, v,e) in enumerate(zip(resistance_seconds, distance_seconds, velocity_seconds, energy_seconds)):
        #print(f"Time = {i} s | Distance = {s:6.2f} m | Speed = {v:6.2f} m/s| Acceleration = {a:6.2f} m/s2| Resistance = {R:.2f} N | Tractice Force at Wheel = {tf:.2f} N")
        print(f"Time = {i} s | Distance = {s:6.2f} m | Speed = {v:6.2f} m/s| Energy = {e:6.2f} kWh")
        total += e
    print(total)
